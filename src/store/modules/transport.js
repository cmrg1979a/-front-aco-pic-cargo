...
const actions = {
  async fetchDataTransport({ commit }, data) {
    ...
    await axios(config)
      .then((response) => {
        ...
        commit("SET_LIST_TRANSPORT", list);
  ...
  async insertTransport({ commit, dispatch }, data) {
    var config = {
      method: "post",
    ...
    await axios(config)
      .then((response) => {
        ...
        Swal.fire({
          ...
          dispatch("fetchDataTransport", data);
        });
      })
      .catch((error) => {
        console.log("Error al obtener los datos:", error);
  ...
  async readTransport({ commit }, id) {
    ...
    await axios(config)
      .then((response) => {
        ...
        sessionStorage.setItem("auth-token", data.token);
        if (data.estadoflag) {
          commit("SET_TRANSPORT", data.data[0]);
        } else {
          Swal.fire({
            icon: !!data.estadoflag ? "success" : "error",
            text: data.mensaje,
  ...
  async updateTransport({ dispatch }, data) {
    var config = {
      method: "put",
  ...
  async cargarTransport({ commit }, data) {
    var config = {
      method: "get",
      url:
        process.env.VUE_APP_URL_MAIN +
