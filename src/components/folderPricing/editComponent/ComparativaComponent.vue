            ...
            <tr v-for="(opcion, index) in opcionesSeleccionadas" :key="index">
              ...
              <td>
                <v-checkbox
                  label=""
                  class="mx-auto"
                  :disabled="bloquearSeleccionada(opcion)"
                  v-model="opcion.selected"
                  @click="recalcularOpciones()"
                >
                </v-checkbox>
              </td>
              <!-- <td>{{ opcion }}</td> -->
...
export default {
  ...
  methods: {
    ...
    recalcularOpciones() {
      ...
      this.$store.state.pricing.reCalcularOpciones =
        !this.$store.state.pricing.reCalcularOpciones;
    },
    ...
    montoConfeccion(item) {
      ...
      if (item.code_cost == 33) {
        ...
        let cif =
          parseFloat(amount) +
          parseFloat(this.this.totalFleteVentas) +
          (parseFloat(amount) + parseFloat(this.this.totalFleteVentas)) * 0.01;
