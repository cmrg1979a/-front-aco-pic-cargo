      ...
      <v-col cols="12">
            ...
            <tr v-for="(opcion, index) in opcionesSeleccionadas" :key="index">
              ...
              <td>
                <v-checkbox
                  ...
                  @click="
                    $store.state.pricing.recargarCostoSeleccionados =
                      !$store.state.pricing.recargarCostoSeleccionados
                  "
                >
                </v-checkbox>
        ...
        >
          <template v-slot:[`item.tarifa`]="{ item }">
            Opción {{ item.nro_propuesta }}
          </template>
          <template v-slot:[`item.selected`]="{ item }">
            <v-checkbox v-model="item.selected"></v-checkbox>
          </template>
          <template v-slot:[`item.tiempo_transito`]="{ item }">
            {{ item.tiempo_transito }} días
          </template>
        </v-data-table> -->
      </v-col>
      ...
      if (this.$store.state.pricing.opcionCostos.some((v) => v.selected)) {
        Swal.fire({
          ...
          if (action.isConfirmed) {
            ...
            this.$store.state.pricing.recargarCostoSeleccionados =
              !this.$store.state.pricing.recargarCostoSeleccionados;
