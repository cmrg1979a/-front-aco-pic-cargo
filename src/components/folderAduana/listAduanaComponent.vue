              ...
              <v-tooltip top>
                <template v-slot:activator="{ on, attrs }">
                  <v-btn
                    ...
                    v-on="on"
                    v-bind="attrs"
                    @click="ira('EditarAduana', item.id)"
...
export default {
  ...
  methods: {
    ...mapActions([
      ...
      "imprimirMercadeo",
      "actualizarAduanaRecibidoEnviado",
    ...
    async updateQuoteRecibidoEnviado() {
      if (this.$refs.frmEstado.validate()) {
        ...
        await this.actualizarAduanaRecibidoEnviado(data);
    ...
    async guardarNota() {
      if (this.$refs.detallenota.validate()) {
        ...
        await this.guardarNotaAduana(data).catch((err) => {
          console.log(err);
        });
        await this.getListQuoteAduana().catch((err) => {
          console.log(err);
        });
    ...
    async eliminar(id, codigo) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          await vm.getListQuoteAduana().catch((err) => {
            console.log(err);
          });
