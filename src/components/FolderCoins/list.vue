<template>
  <v-card fluid class="px-5">
    ...
    >
      <v-list-item>
        <v-list-item-content>
          <v-list-item-title class="text-h6"> Nueva Moneda </v-list-item-title>
        </v-list-item-content>
      </v-list-item>
        ...
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title class="text-h6">
              Ver: {{ $store.state.coins.record.name }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        ...
        <v-card-text>
          <v-row>
            <v-text-field
              label="Código"
              readonly
              :value="$store.state.coins.record.code"
            >
            </v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Símbolo"
              readonly
              :value="$store.state.coins.record.symbol"
            >
            </v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Acrónimo"
              readonly
              :value="$store.state.coins.record.acronym"
            >
            </v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Nombre"
              readonly
              :value="$store.state.coins.record.name"
            >
            </v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Descripción"
              readonly
              :value="$store.state.coins.record.description"
            >
            </v-text-field>
          </v-row>
          <v-row>
            <v-switch
              readonly
              :label="`Estado: ${
                !!$store.state.coins.record.status ? 'Activo' : 'Inactivo'
              }`"
              color="success"
              v-model="$store.state.coins.record.status"
            ></v-switch>
          </v-row>
        </v-card-text>
        <v-card-actions class="justify-end mt-3">
    ...
    >
      <v-container v-if="!loading_as">
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title class="text-h6">
              Modificar: {{ $store.state.coins.record.name }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
...
export default {
  ...
  async mounted() {
    await this.fetchDataCoins(this.records);
  ...
  methods: {
    ...
    async submit_insert() {
      if (this.$refs.formNew.validate()) {
        this.resetRecord("filter");
        await this.insertCoins(this.record);
      }
    },
    async submit_update() {
    ...
    async submit_filter() {
      ...
      await this.fetchDataCoins(data);
      this.showFilterDrawer = false;
    },
    validateSymbol(v) {
      if (!v) {
      ...
      if (v.length > 255) {
        return "El símbolo permite hasta 255 caracteres";
      }
      return true;
    },
    validateAcronym(v) {
      ...
      if (v.length > 255) {
        return "El acrónimo permite hasta 255 caracteres";
      }
      return true;
    },
    validateName(v) {
  ...
  watch: {
    "$store.state.coins.record": {
      handler(newRecord, oldRecord) {
        if (newRecord && Object.keys(newRecord).length > 0) {
          const { code, symbol, name, description, acronym, status } =
            newRecord;
          this.record = { code, symbol, name, description, acronym, status };
          this.loading_as = false;
        }
      },
      immediate: true,
    },
    "$store.state.coins.loading": {
