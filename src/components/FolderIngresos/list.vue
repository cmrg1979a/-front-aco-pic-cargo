    ...
    >
      <v-container v-if="!loading_as">
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title class="text-h6">
              Ver: {{ $store.state.ingresos.record.description }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        ...
        <v-card-text>
          <v-row>
            <v-text-field
              label="Código"
              readonly
              :value="$store.state.ingresos.record.code"
            >
            </v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Descripción"
              readonly
              :value="$store.state.ingresos.record.description"
            >
            </v-text-field>
          </v-row>
          <v-row>
            <v-checkbox
              label="Se calcula en Reportes"
              readonly
              v-model="$store.state.ingresos.record.calculoflag"
            ></v-checkbox>
          </v-row>
          <v-row>
            <v-switch
              readonly
              :label="`Estado: ${
                !!$store.state.ingresos.record.status ? 'Activo' : 'Inactivo'
              }`"
              color="success"
              v-model="$store.state.ingresos.record.status"
            ></v-switch>
          </v-row>
        </v-card-text>
        <v-card-actions class="justify-end mt-3">
    ...
    >
      <v-container v-if="!loading_as">
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title class="text-h6">
              Modificar: {{ $store.state.ingresos.record.description }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
...
export default {
  ...
  async mounted() {
    await this.fetchDataIngreso(this.records);
    this.$store.state.mainTitle = `Tipo de Ingreso`;
  },
  methods: {
    ...mapActions([
      "fetchDataIngreso",
    ...
    submit_insert() {
      ...
      setTimeout(async () => {
        if (this.$refs.formNew.validate() && this.errorCode == null) {
          this.resetRecord("filter");
          await this.insertIngreso(this.record);
        }
      }, 500);
    },
    async submit_update() {
    ...
    async submit_filter() {
      ...
      await this.fetchDataIngreso(data);
      this.showFilterDrawer = false;
    },
    validateCode(v) {
      if (!v) {
  ...
  watch: {
    "$store.state.ingresos.record": {
      handler(newRecord, oldRecord) {
        if (newRecord && Object.keys(newRecord).length > 0) {
          const { code, calculoflag, description, status } = newRecord;
          this.record = { code, calculoflag, description, status };
          this.loading_as = false;
        }
      },
      immediate: true,
    },
    "$store.state.ingresos.loading": {
