  ...
  <v-card fluid class="px-5">
    <v-card elevation="0">
      ...
      <v-card-title v-if="!loading" class="elevation-0">
        ...
        <v-spacer></v-spacer>
        <!-- <v-btn small color="success" @click="open_new()">
          Nuevo
        </v-btn> -->
      ...
      <v-data-table
        ...
        class="elevation-1"
        :headers="headers"
        :items="$store.state.modality.list"
        item-key="id"
        :search="search"
      >
        ...
        <template v-slot:[`item.action`]="{ item }">
          <!-- <v-btn icon>
            <v-icon  color="warning" @click="open_update(item.id)">mdi-pencil</v-icon>
          </v-btn> -->
          <v-btn icon>
    ...
    >
      <v-container v-if="!loading_as">
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title class="text-h6">
              Ver: {{ $store.state.modality.record.name }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        ...
        <v-card-text>
          <v-row>
            <v-text-field
              label="Código"
              readonly
              :value="$store.state.modality.record.code"
            ></v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Nombre"
              readonly
              :value="$store.state.modality.record.name"
            ></v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Descripción"
              readonly
              :value="$store.state.modality.record.description"
            ></v-text-field>
          </v-row>
          <v-row>
            <v-switch
              readonly
              :label="`Estado: ${
                !!$store.state.modality.record.status ? 'Activo' : 'Inactivo'
              }`"
              color="success"
              v-model="$store.state.modality.record.status"
            ></v-switch>
          </v-row>
        </v-card-text>
    ...
    >
      <v-container v-if="!loading_as">
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title class="text-h6">
              Modificar: {{ $store.state.modality.record.name }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
    ...
    >
      ...
      <v-container>
        <v-card-actions class="justify-end mt-3">
          <v-btn small class="mx-1" color="success" @click="submit_filter()"
            >Aceptar</v-btn
          >
...
export default {
  ...
  async mounted() {
    await this.fetchDataModality(this.records);
    this.$store.state.mainTitle = `Modalidad`;
  },
  methods: {
    ...mapActions([
      "fetchDataModality",
    ...
    async submit_insert() {
      if (this.$refs.formNew.validate()) {
        ...
        await this.insertModality(this.record);
      }
    },
      ...
      if (this.record.status == 0) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            this.resetRecord("filter");
            await this.updateModality(data);
          } else {
            item.status = !status;
          }
        });
    ...
    async submit_filter() {
      ...
      await this.fetchDataModality(data);
      this.showFilterDrawer = false;
    },
    validateCode(v) {
      if (!v) {
