  ...
  <v-card fluid class="px-5">
    <v-card elevation="0">
      ...
      <v-card-title v-if="!loading" class="elevation-0">
        ...
        <v-btn small dark class="mx-1" color="#2962FF" @click="open_filter()">
          Fitrar
        </v-btn>
    ...
    >
      <v-container v-if="!loading_as">
        <v-list-item class="px-0">
          <v-list-item-content>
            <v-list-item-title class="text-h6">
              Ver: {{ $store.state.groupservices.record.name }}
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        ...
        <v-card-text>
          <v-row>
            <v-text-field
              label="Código"
              readonly
              :value="$store.state.groupservices.record.code"
            ></v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Nombre"
              readonly
              :value="$store.state.groupservices.record.name"
            ></v-text-field>
          </v-row>
          <v-row>
            <v-text-field
              label="Descripción"
              readonly
              :value="$store.state.groupservices.record.description"
            ></v-text-field>
          </v-row>
          <v-row>
            <v-switch
              readonly
              :label="`Estado: ${
                !!$store.state.groupservices.record.status
                  ? 'Activo'
                  : 'Inactivo'
              }`"
              color="success"
              v-model="$store.state.groupservices.record.status"
            ></v-switch>
          </v-row>
        </v-card-text>
        <v-card-actions class="justify-end mt-3">
      ...
      <v-container v-if="!loading_as">
        <v-card-title>
          <v-list-item class="px-0">
            <v-list-item-content>
              <v-list-item-title class="text-h6">
                Modificar: {{ $store.state.groupservices.record.name }}
              </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
    ...
    >
      <v-container>
        ...
        <v-card-actions class="justify-end mt-3">
          <v-btn class="mx-1" color="success" @click="filter()">
            Aceptar
          </v-btn>
          <v-btn class="mx-1" color="default" @click="reset_records()">
            Limpiar</v-btn
          >
          <v-btn
            class="mx-1"
...
export default {
  name: "ListGroupservicesComponent",
  data() {
    return {
      headers: [
  ...
  async mounted() {
    await this.fetchDataGroupServices(this.records);
    this.$store.state.mainTitle = `Servicio`;
  },
  methods: {
    ...mapActions([
      "fetchDataGroupServices",
      "insertGroupServices",
      "readGroupServices",
      "updateGroupServices",
    ]),
    ...
    async insert() {
      if (this.$refs.formNew.validate()) {
        ...
        await this.insertGroupServices(this.record);
      }
    },
    ...
    async switch_status(item) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          item.status = 0;
          this.resetRecord("filter");
          await this.updateGroupServices(item);
        }
      });
    },
    async open_read(id) {
      ...
      await this.readGroupServices(id);
    },
    ...
    async btnUpdate() {
      ...
      if (this.$refs.form.validate()) {
        this.resetRecord("filter");
        await this.updateGroupServices(data);
      }
    },
    open_new() {
      this.resetRecord();
    ...
    async filter() {
      ...
      await this.fetchDataGroupServices(data);
      this.showFilterDrawer = false;
    },
    validateName(v) {
      let length = 50;
      if (!v) {
        ...
        return `El nombre permite hasta ${length} caracteres`;
      } else {
        return true;
      }
    },
    validateDescription(v) {
      ...
      if (!v) {
        ...
        return `La descripción permite hasta ${length} caracteres`;
      } else {
        return true;
      }
    },
  ...
  watch: {
    "$store.state.groupservices.record": {
    ...
    "$store.state.groupservices.loading": {
